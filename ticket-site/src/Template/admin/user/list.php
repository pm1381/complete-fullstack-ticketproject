...
<div class="adminDataShow">
    ...
    <div class="px-4 pb-2 mainContent">
        <?php //print_f($this->data['sortStyle']); ?>
        <?php if (Tools::checkArray($this->data, 'haveAlert')) { ?>
            <div class="alert alert-info" role="alert" style="text-align: center; margin-top:1em">
                <?php echo 'تغییرات انجام شد' ?>
            </div>
        <?php } ?>
        <div class="whereData">
            <div class="userPage serachUser searchBarMargins">
                <input class="searchInput" placeholder="موبایل یا کد رزرو ..." name="searchInput">
                <div class="searchButton searchUser">
            ...
            <div class="sort">
                <form method="GET">
                    ...
                    <select id="sort" name="sortStyle" class="sortStyle" onchange=userSortChange(this.value)>
                        <option data-name="انتخاب کنید" value="0">انتخاب کنید</option>
                        <?php echo Form::option(Arrays::userPageSort(), $this->data['sortStyle']) ?>
                    </select>
                </form>
            </div>
        </div>
        <table cellspacing="0" cellpadding="0" border="0" class="table table-striped table-bordered">
            <tbody>
                <tr class="title">
                    <td width="70px" class="titleOptions">ردیف</td>
                    ...
                    <td class="titleOptions">شب حضور</td>
                    <td class="titleOptions">پرداختی</td>
                    <td class="titleOptions">پیامک</td>
                    <td class="titleOptions">کد</td>
                </tr>
                <?php $i = 0; ?>
                <?php foreach ($this->data['userResult'] as $userData) {
                    $sms = '<td style="text-align:center">ارسال شده</td>';
                    if ($userData['userSms'] == 0) {
                        $sms = '
                            <td class="options">' .
                                '<a href="./sendSms/' . $userData['userId'] . '/' . '" class="settings sendSms"   user-id=' . $userData['userId'] . '>ارسال پیامک</a>' 
                            . '</td>
                        ';
                    }
                    echo
                    '
                        <tr class="record record-'. $userData['userId'] .'">
                            <td class="options">' . ++$i . '</td>
                            <td class="options">' . $userData['userName']  . '</td>
                            <td class="options">' . $userData['userMobile'] . '</td>
                            <td class="options">' . $userData['userTicketCount'] . '</td>
                            <td class="options">' . $userData['userCode'] . '</td>
                            <td class="options">' . $userData['userTheater'] . '</td>
                            <td class="options">' . $userData['userSum'] . '</td>
                            '. $sms .'
                            <td class="options">' .
                                '<div class="settings giveReserveCode"   user-id=' . $userData['userId'] . '>کد تصادفی</div>' 
                            . '</td>
                        </tr>
                    ';
                } ?>
                <?php if (count($this->data['userResult']) == 0) { ?>
