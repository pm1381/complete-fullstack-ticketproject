...
<div class="adminDataShow">
    ...
    <div class="px-4 pb-2 mainContent">
        <form method="GET">
            <div class="serachUser searchBarMargins">
                <input class="searchInput" placeholder="... موبایل یا کد رزرو یا شماره کارت 16 رقمی" name="searchInput">
                <button class="cardSearch">
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="table table-striped table-bordered">
            <tbody>
                <tr class="title">
                    ...
                    <td class="titleOptions">شماره کارت</td>
                    ...
                    <td class="titleOptions">انجام</td>
                </tr>
                <?php foreach ($this->data['refundResult'] as $refund) {
                    if ($refund['cardActive']) {
                        $options = '
                            <td class="options">' .
                                '<div class="settings-done refundDone" user-id=' . $refund['cardId'] . '>انجام</div>' 
                            . '</td>
                        ';
                    } else {
                        $options = '
                            <td class="options">' .
                                '<div class="settings-done refundDone" user-id=' . $refund['cardId'] . ' style="background-color:#198754">انجام شده</div>' 
                            . '</td>
                        ';
                    }
                    echo
                    '
                        <tr>
                            <td class="options">' . $refund['cardId'] . '</td>
                            <td class="options">' . $refund['cardNumber']  . '</td>
                            <td class="options">' . $refund['userName'] . '</td>
                            <td class="options">' . $refund['userMobile'] . '</td>
                            <td class="options">' . $refund['userTicketCount'] . '</td>
                            <td class="options">' . $refund['userCode'] . '</td>
                            <td class="options">' . $refund['userSum'] . '</td>
                            ' . $options . '
                        </tr>
                    ';
                } ?>
                <?php if (count($this->data['refundResult']) == 0) { ?>
                    <?php echo '<td colspan="20" style="text-align:center">موردی یافت نشد</td>' ?>
                <?php } ?>
            </tbody>
        </table>
    </div>
